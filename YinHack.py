...
def send_infinite(receiver,sentences):
    ...
    server.starttls()  #encrypts following data
    server.login('username', "password") #enter own info
    
    for i in range(0,999): 
        msg = MIMEText(create_message(sentences))
        msg['Subject'] = create_message(1)
        server.sendmail("username", receiver, msg.as_string())  #enter sending email
        sleep(1) 
